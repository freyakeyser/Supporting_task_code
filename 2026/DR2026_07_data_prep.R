...
logs_and_fish(loc="offshore", year=2025, get.marfis = F, export = F, direct = direct)
...
write.csv(mersey, "Y:/Offshore/Assessment/2026/Supporting_tasks/Mersey_DR2026_07/Mersey_logs_2025.csv")
...
fish_sf$ID.1 <- gsub(x=fish_sf$ID.1, pattern=".shp", replacement="", fixed=T)
...
fish_sf$period <- cut(fish_sf$year, breaks=seq(2010,2025, 5), right=F)
...
dim(log_mike)
sum(fish_sf$pro.repwt[fish_sf$year==2025])
sum(log_mike$pro.repwt[log_mike$year==2025])
sum(poly_kg$pro.repwt[poly_kg$year==2025], na.rm=T)
unique(poly_kg$pro.repwt[poly_kg$year==2025 & poly_kg$CAN_SHOW=="NO"])
dim(poly_kg)
...
print(bp + 
        #geom_sf(data=foot_grid[!is.na(foot_grid$kg) & !is.na(foot_grid$year),], fill="transparent", colour=NA, show.legend=T) +
        geom_sf(data=log_mike, aes(colour=sfa), show.legend=T) +
        #geom_text(data=compare[!is.na(compare$year),], aes(x=-Inf, y=-Inf, label=paste0("kg screened in: ", round(screen_kg,0), "\nkg screened out: ", round(diff_kg,0))), hjust = -0.1, vjust = -0.1)+
        geom_sf(data=poly_kg[poly_kg$CAN_SHOW=="NO",], fill="black", alpha=0.1)+
        theme(legend.position="right") +
        scale_fill_viridis_c(name=paste0("Catch", " (t)"), option="rocket", begin=0.95, end=0)+
...
write.csv(x=log_mike, file = "Y:/Offshore/Assessment/2026/Supporting_tasks/Mersey_DR2026_07/logdata_2025_gridded_screened_Science.csv")
