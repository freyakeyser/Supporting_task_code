...
if(length(discards_effort_2_lost)>0){
  lost <- NULL
...
species_cols <- names(discards_report)[which(!names(discards_report) %in% c("VESSEL_NAME", "TRIP", "obshooks", "vrnum", "totalhooks", "hm", 
...
species_cols <- names(species_cols)[which(!names(monthly_eff) %in% c("bank","year", "month", "obshooks", "totalhooks", "obseffort", "prophooks"))]
...
if(is.character(new.log.dat$month)) new.log.dat$month <- month(new.log.dat$date)
...
if(all(length(banks)>1 | !banks=="all")) {
  ...
  monthly$bank <- paste0(banks, collapse=" ")
}
...
common <- monthly %>% group_by(COMMON) %>%
  summarize(annual=sum(totaldiscards, na.rm=T)) %>%
  filter(annual>0) %>%
  filter(!COMMON %in% c("SEA SCALLOP", "STONES AND ROCKS", "FOREIGN ARTICLES,GARBAGE", "SCALLOP SHELLS", "SAND")) %>%
  arrange(-annual, COMMON)
# most common discards: WINTER SKATE, BARNDOOR SKATE, MONKFISH,GOOSEFISH,ANGLER, ASTEROIDEA S.C., LITTLE SKATE, BASKET STARS, 
# MUSSELS (NS), WINTER FLOUNDER, SEA URCHINS, HERMIT CRABS, AMERICAN LOBSTER, THORNY SKATE, WHELKS, SEA RAVEN, YELLOWTAIL FLOUNDER, 
# JONAH CRAB, LONGHORN SCULPIN, COD(ATLANTIC), BRITTLE STAR, SPONGES, HADDOCK, SQUIRREL OR RED HAKE, SNAILS AND SLUGS, MARINE INVERTEBRATES (NS),
# WHITE HAKE, SILVER HAKE, SPINY DOGFISH, BAY SCALLOP, FIG SPONGE, ATLANTIC ROCK CRAB, SEA CUCUMBERS, WITCH FLOUNDER, SMOOTH SKATE, ASTERIAS SP., 
# SEA ANEMONE, OCEAN POUT(COMMON), SCULPINS, AMERICAN PLAICE, SKATES (NS), SAND DOLLARS,
common <- common$COMMON[1:30]

ggplot() + geom_line(data=monthly[monthly$COMMON %in% common,], aes(ymd(paste0(year, "-", month, "-", 15)), discardrate)) + facet_wrap(~COMMON)+
  ylab("discard rate (kg/hm)")+
  xlab("month")

ggplot() + geom_line(data=monthly[monthly$COMMON %in% common,], aes(ymd(paste0(year, "-", month, "-", 15)), totaldiscards)) + facet_wrap(~COMMON)+
  ylab("kg discarded by the fleet")+
  xlab("month")

require(mgcv)
monthly$Time <- as.numeric(ymd(paste0(monthly$year, "-", monthly$month, "-15")))/1000

dr_gam <- NULL
sig <- NULL
preds <- NULL
for(i in common){
  temp <- monthly[monthly$COMMON %in% i & !is.na(monthly$discardrate),]
  dr_gam[[i]] <- gam(data=temp, discards ~ obseffort + s(month, bs="cc") + s(Time), family=betar(link = "logit"))
  pval <- as.data.frame(summary(dr_gam[[i]])$s.table)["s(month)", "p-value"]
  if(pval < 0.05) sig <- c(sig, i)
  pred.dat <- data.frame(common=i, month=1:12, obseffort=1, Time=as.numeric(ymd("2025-01-01"))/1000)
  preds_i <- cbind(pred.dat, as.data.frame(predict(dr_gam[[i]], newdata=pred.dat, se.fit=T)))
  preds <- rbind(preds, preds_i)
}

preds$fit.resp <- boot::inv.logit(preds$fit)
preds$LCI <- boot::inv.logit(preds$fit - 1.96* preds$se.fit)
preds$UCI <- boot::inv.logit(preds$fit + 1.96* preds$se.fit)

ggplot() + geom_line(data=preds[preds$common %in% sig,], aes(month, fit)) + 
  ylab("discard rate (kg/hm)")+
  facet_wrap(~common, scales='free_y')

season <- preds[preds$common %in% sig,] %>%
  group_by(common) %>%
  slice(which.max(fit)) %>%
  arrange(month) %>%
  mutate(peak = ifelse(month %in% 1:3, 1, ifelse(month %in% 4:6, 2, ifelse(month %in% 7:9, 3, ifelse(month %in% 10:12, 4,NA))))) %>%
  select(-month, -fit, -se.fit, -UCI, -LCI, -obseffort, -fit.resp, -Time) %>%
  right_join(preds[preds$common %in% sig,])

ggplot() + geom_ribbon(data=season, aes(month, ymin=LCI, ymax=UCI), fill="grey")+
  geom_line(data=season, aes(month, boot::inv.logit(fit))) +
  ylab("discard rate (kg/hm)")+
  facet_wrap(~peak+common, scale="free")
